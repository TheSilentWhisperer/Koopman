...
for video_name in tqdm(os.listdir(train_path)):
    if 'mp4' in video_name:
        ...
        #also write the q_i.json file
        q_i_path = os.path.join(train_path, "q_" + video_name.split('.')[0] + ".json")
        with open(q_i_path, 'r') as f:
            q_i = json.load(f)
        with open(os.path.join(target_train_path, "q_" + video_name.split('.')[0] + ".json"), 'w') as f:
            json.dump(q_i, f)
...
for video_name in tqdm(os.listdir(test_path)):
    if 'mp4' in video_name:
        ...
        clip_resized.write_videofile(target_path, audio=False)
        #also write the q_i.json file
        q_i_path = os.path.join(test_path, "q_" + video_name.split('.')[0] + ".json")
        with open(q_i_path, 'r') as f:
            q_i = json.load(f)
        with open(os.path.join(target_test_path, "q_" + video_name.split('.')[0] + ".json"), 'w') as f:
            json.dump(q_i, f)
...
if 'eval' in os.listdir(SOURCE_DIR):
    ...
    for video_name in tqdm(os.listdir(eval_path)):
        if 'mp4' in video_name:
            ...
            clip_resized.write_videofile(target_path, audio=False)
            #also write the q_i.json file
            q_i_path = os.path.join(eval_path, "q_" + video_name.split('.')[0] + ".json")
            with open(q_i_path, 'r') as f:
                q_i = json.load(f)
            with open(os.path.join(target_eval_path, "q_" + video_name.split('.')[0] + ".json"), 'w') as f:
                json.dump(q_i, f)